...
using Alpha1Financial.ClientLibraries;
using Alpha1Financial.Common.Data;
using System;
...
namespace InfragisticsMigration
{
	public class DataEntryGrid : UltraGrid
	{
		...
		private void worker_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				...
				Factory.SettingSystemAsync.SaveSettingStreamWithLang(keyValuePair.Key, Global.CurrentUser, "", CurrrentUserLang ,keyValuePair.Value.ToArray());
			}
			catch
			{
			}
		...
		private string CurrrentUserLang
		{
			get
			{
				var userLang = LocalSettings.GlobalSettings.GetSetting("CurrentLanguage");
				if (userLang is null)
				{
					return "English";
				}
		...
		public bool LoadLayout()
		{
			try
			{
				...
				byte[] binaryData = Factory.SettingSystemAsync.GetBinaryDataWithUserLang(Global.CurrentUser, key, CurrrentUserLang);
				if (binaryData == null)
				{
					return false;
				}
